...
def test_delete_project_endpoint(client) -> None:
    token = register_and_login(client, "delete-project@test.com")
    headers = auth_headers(token)

    create = client.post("/projects", json={"name": "Delete Me", "description": "x"}, headers=headers)
    project_id = create.json()["id"]

    delete_res = client.delete(f"/projects/{project_id}", headers=headers)
...
def test_delete_artifact_endpoint(client) -> None:
    token = register_and_login(client, "delete-artifact@test.com")
    headers = auth_headers(token)

    project = client.post("/projects", json={"name": "Delete Artifact", "description": "x"}, headers=headers)
    project_id = project.json()["id"]

    before = client.post(
        f"/projects/{project_id}/model-versions",
        headers=headers,
        files={"file": ("before.json", _fixture_bytes(_payload("sig1")), "application/json")},
        data={"label": "before", "source_type": "normalized_json"},
    )
    after = client.post(
        f"/projects/{project_id}/model-versions",
        headers=headers,
        files={"file": ("after.json", _fixture_bytes(_payload("sig2")), "application/json")},
    ...
    delete_res = client.delete(f"/artifacts/{artifact_id}", headers=headers)
    assert delete_res.status_code == 200
    assert delete_res.json()["deleted"] is True
