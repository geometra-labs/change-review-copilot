...
def test_html_export_artifact(client) -> None:
    token = register_and_login(client, "html-export@test.com")
    headers = auth_headers(token)

    project = client.post("/projects", json={"name": "HTML Export", "description": "x"}, headers=headers)
    ...
    export_res = client.post(f"/comparisons/{comparison_id}/export?format=html", headers=headers)
    assert export_res.status_code == 200
    assert export_res.json()["artifact_type"] == "html"


def test_pdf_export_downloads_valid_pdf_header(client) -> None:
    token = register_and_login(client, "pdf-export@test.com")
    headers = auth_headers(token)

    project = client.post("/projects", json={"name": "PDF Export", "description": "x"}, headers=headers)
    project_id = project.json()["id"]
    ...
    export_res = client.post(f"/comparisons/{comparison_id}/export?format=pdf", headers=headers)
    assert export_res.status_code == 200
    assert export_res.json()["artifact_type"] == "pdf"
